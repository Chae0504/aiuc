...
#SBATCH -o /home/yoonjoo_chae/AIUC/logs/slurm/rnncell_strict_transition_2gpu_%j.out
...
TRANSITION_WEIGHT="${TRANSITION_WEIGHT:-0.5}"
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-64}"
JOB_ID="${SLURM_JOB_ID:-manual}"
OUTPUT_DIR="AIUC/outputs/rnncell_strict_transition_w${TRANSITION_WEIGHT}_2gpu_${JOB_ID}"
DATA_PATH="AIUC/DG/uc_new_data_strict.npz"
SPECS_PATH="AIUC/DG/generator_specs.csv"

python - "$TRANSITION_WEIGHT" "$GLOBAL_BATCH_SIZE" <<'PY'
import math
import sys

transition_weight = float(sys.argv[1])
global_batch_size = int(sys.argv[2])
expected_gpus = 2
if not math.isfinite(transition_weight) or transition_weight < 0:
    raise ValueError("TRANSITION_WEIGHT must be a finite non-negative number")
...
print(f"Transition loss weight: {transition_weight:g}")
print(
...
python -u AIUC/train_rnncell_strict_transition.py \
  --data "$DATA_PATH" \
  --specs "$SPECS_PATH" \
  ...
  --expected-gpus 2 \
  --status-loss-mode transition_bce \
  --status-transition-loss-weight "$TRANSITION_WEIGHT" \
  --phase2-epochs 150 \
  --phase2-patience 40 \
